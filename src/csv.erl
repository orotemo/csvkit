...
%%% Parse csv line to a list according to its columns
%%% work with comma separated value records,
%%% when fields can contain quotes
%% source originaly taken from:
%%% http://www2.erlangcentral.org/wiki/index.php?title=Comma_Separated_Values
%%%
-export([read/1]).
...
read([13, 10|String], Acc) ->
  ...
  read(Rest, [Line|Acc]);
read([13|String], Acc) ->
 io:format(user,"~p, ~p~n",[String,Acc]),
  read(String, Acc).
